...
import { z } from 'zod'
import { oc, type InferContractInputs, type InferContractOutputs } from '@ts-pf/contract'
...
describe('contract infer types', () => {
  ...
  it('defaults omitted output to unknown and omitted input to void', () => {
    ...
    expectTypeOf<InferContractOutputs<{ x: typeof noOut }>['x']>().toEqualTypeOf<unknown>()

    const noIn = oc.output(z.string())
    expectTypeOf<InferContractInputs<{ x: typeof noIn }>['x']>().toEqualTypeOf<void>()
  })
})
